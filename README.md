...
# LLM 기반 확장 가능한 AI 성격 제어 시스템 (Personality Vector Merging)
Scalable Personality Control for LLM-based Agents via Personality Vector Merging

> **요약(한 줄)**: 프롬프트가 아닌 **모델 파라미터 차이(Personality Vector)**로 성격을 정의하고, **α 스케일링 + 모델 병합**으로 성격 강도/조합을 연속적으로 제어합니다.  
> **One-liner**: Define personality as parameter deltas and enable continuous control via α-scaling and model merging.
...
- **α 스케일링**으로 성격 강도를 연속적으로 조절  
  Control intensity continuously using α-scaling.
- 다중 성격 조합을 **모델 병합(merging)**으로 확장 가능하게 설계  
...
![architecture](./image/figure1_git.png)
...
> 아래 커맨드는 **예시**이며, 실제 모델 경로/환경에 맞게 수정하세요.  
